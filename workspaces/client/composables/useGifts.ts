export const useGifts = () => {
  ...
  const period = (item: any): string => {
    if (item.period) return item.period.name
    if (item.until) return $t('cabinet.gift_until', { date: $moment(item.until).format('D MMMM YYYY') })

    return $t('cabinet.gift_forever')
  }
  ...
  const describe = (item: any): string => {
    ...
    if (item.donate_group)
      return $t('cabinet.gift_donate', { name: item.donate_group.name, period: period(item), server: item.server.name })

    if (item.donate_permission)
      return item.donate_permission.type == 'web'
        ? $t('cabinet.gift_permission_web', { name: item.donate_permission.name, period: period(item) })
        : $t('cabinet.gift_permission', { name: item.donate_permission.name, period: period(item), server: item.server.name })
