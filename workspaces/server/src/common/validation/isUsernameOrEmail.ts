  ...
  return ValidateBy(
    {
      ...
      constraints: [options],
      validator: {
        validate: (value, args): boolean => isUsername(value) || isEmail(value, args.constraints[0]),
        defaultMessage: buildMessage((eachPrefix) => `${eachPrefix}$property must be an username or email`, validationOptions),
      },
    },
    validationOptions,
  );
